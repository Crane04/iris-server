...
const playgroundLimiter = rateLimit({
  ...
  legacyHeaders: false,
  message: { error: "Playground rate limit reached. Sign up for an API key to get higher limits." },
  keyGenerator: (req) => {
...
router.post("/compare", playgroundLimiter, async (req: Request, res: Response): Promise<void> => {
  try {
